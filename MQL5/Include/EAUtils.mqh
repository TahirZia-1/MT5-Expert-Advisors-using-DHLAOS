...
//|                                                                  |
//+------------------------------------------------------------------+
// Replace the IsFillingTypeAllowed function in EAUtils.mqh with this corrected version
bool IsFillingTypeAllowed(string symbol, ENUM_ORDER_TYPE_FILLING fill_type) {
    ...
    // For market execution mode (like Pepperstone), RETURN filling is typically required
    if (exec_mode == SYMBOL_TRADE_EXECUTION_MARKET) {
        return fill_type == ORDER_FILLING_RETURN;
    }
    
    // For instant execution, FOK is usually required
    if (exec_mode == SYMBOL_TRADE_EXECUTION_INSTANT) {
        return fill_type == ORDER_FILLING_FOK;
    }
...
bool order(ENUM_ORDER_TYPE ot, ulong magic, double in, double sl = 0, double tp = 0, double risk = 0.01, bool martingale = false, double martingaleRisk = 0.04, int slippage = 30, bool isl = false, bool itp = false, string comment = "", string name = NULL, double vol = 0, int nRetry = 5, int mRetry = 2000, bool news = false, ENUM_NEWS_IMPORTANCE newsImportance = NEWS_IMPORTANCE_MEDIUM, int newsMinsBefore = 60, int newsMinsAfter = 60, ENUM_FILLING filling = FILLING_DEFAULT, ENUM_RISK risk_mode = RISK_DEFAULT) {
    ...
    Print("Symbol: ", name, 
          " | Execution Mode: ", SymbolInfoInteger(name, SYMBOL_TRADE_EXEMODE),
          " | Filling Mode: ", SymbolInfoInteger(name, SYMBOL_FILLING_MODE),
          " | Recommended Filling: ", EnumToString(GetRecommendedFillingMode(name)));
...
// In the order function, replace the Pepperstone section with this:
if (IsPepperstoneBroker()) {
    slippage = MathMax(slippage, 50);
    
    // Use the recommended filling mode for Pepperstone
    ENUM_ORDER_TYPE_FILLING recommendedMode = GetRecommendedFillingMode(name);
    filling = (recommendedMode == ORDER_FILLING_FOK) ? FILLING_FOK :
              (recommendedMode == ORDER_FILLING_IOC) ? FILLING_IOK :
              (recommendedMode == ORDER_FILLING_BOC) ? FILLING_BOC :
              FILLING_RETURN;
    
    Print("Using filling mode: ", EnumToString(recommendedMode), " for ", name);
}
...
// Replace the current filling logic with this:
if (filling == FILLING_DEFAULT) {
    req.type_filling = GetRecommendedFillingMode(name);
} else {
    // Convert from ENUM_FILLING to ENUM_ORDER_TYPE_FILLING
    if (filling == FILLING_FOK) req.type_filling = ORDER_FILLING_FOK;
...
// Add a fallback mechanism if the selected filling mode fails
if (!OrderCheck(req, cres) && cres.retcode == TRADE_RETCODE_INVALID_FILL) {
    Print("Selected filling mode not supported, trying alternatives...");
    
    // Try FOK first
    if (req.type_filling != ORDER_FILLING_FOK && 
        (SymbolInfoInteger(name, SYMBOL_FILLING_MODE) & ORDER_FILLING_FOK) == ORDER_FILLING_FOK) {
        req.type_filling = ORDER_FILLING_FOK;
        Print("Trying FOK filling mode instead");
    }
    // Then try IOC
    else if (req.type_filling != ORDER_FILLING_IOC && 
             (SymbolInfoInteger(name, SYMBOL_FILLING_MODE) & ORDER_FILLING_IOC) == ORDER_FILLING_IOC) {
        req.type_filling = ORDER_FILLING_IOC;
        Print("Trying IOC filling mode instead");
    }
    // Then try RETURN
    else if (req.type_filling != ORDER_FILLING_RETURN && 
             (SymbolInfoInteger(name, SYMBOL_FILLING_MODE) & ORDER_FILLING_RETURN) == ORDER_FILLING_RETURN) {
        req.type_filling = ORDER_FILLING_RETURN;
        Print("Trying RETURN filling mode instead");
    }
    // Finally try BOC
    else if (req.type_filling != ORDER_FILLING_BOC && 
             (SymbolInfoInteger(name, SYMBOL_FILLING_MODE) & ORDER_FILLING_BOC) == ORDER_FILLING_BOC) {
        req.type_filling = ORDER_FILLING_BOC;
        Print("Trying BOC filling mode instead");
    }
}
    ...
    do {
        ...
        // After the OrderSend call in the retry loop, add this check:
if (!os && res.retcode == TRADE_RETCODE_INVALID_FILL) {
    // Try with a different filling mode
    if (req.type_filling == ORDER_FILLING_RETURN) {
        req.type_filling = ORDER_FILLING_FOK;
    } else if (req.type_filling == ORDER_FILLING_FOK) {
        req.type_filling = ORDER_FILLING_IOC;
    } else {
        req.type_filling = ORDER_FILLING_RETURN;
    }
    
    Print("Retrying with filling mode: ", EnumToString(req.type_filling));
    
    // Reset the retry counter to give the new filling mode a chance
    cnt = 1;
...
double calcCostByTicket(ulong ticket) {
    if (!PositionSelectByTicket(ticket)) {
        ...
        // Only log errors that aren't "position not found" (which is normal for recently closed positions)
        if (err != ERR_TRADE_POSITION_NOT_FOUND) {
            PrintFormat("%s error #%d : %s", __FUNCTION__, err, ErrorDescription(err));
        }
        return 0;
    }
    ...
    // Only try to access history for closed positions
...
// Add this function in the global scope of EAUtils.mqh, 
// for example near the end of the file before the last closing brace

//+------------------------------------------------------------------+
//| Check if running on Pepperstone broker                           |
//+------------------------------------------------------------------+
bool IsPepperstoneBroker() {
    // Don't apply Pepperstone adjustments in Strategy Tester
    if (MQLInfoInteger(MQL_TESTER)) {
        return false;
    }
    
    string broker = AccountInfoString(ACCOUNT_COMPANY);
    // More specific check for Pepperstone
    return (StringFind(broker, "Pepperstone") != -1 || 
            StringFind(broker, "PEPPERSTONE") != -1 ||
            StringFind(broker, "pepperstone") != -1);
}
//+------------------------------------------------------------------+
ENUM_ORDER_TYPE_FILLING GetRecommendedFillingMode(string symbol) {
    // Get the symbol's execution mode
    int exec_mode = (int)SymbolInfoInteger(symbol, SYMBOL_TRADE_EXEMODE);
    ...
    // Check which filling modes are actually supported
    if ((filling_mode & ORDER_FILLING_FOK) == ORDER_FILLING_FOK) {
        Print("FOK filling is supported");
        return ORDER_FILLING_FOK;
    }
    if ((filling_mode & ORDER_FILLING_IOC) == ORDER_FILLING_IOC) {
        Print("IOC filling is supported");
        return ORDER_FILLING_IOC;
    }
    if ((filling_mode & ORDER_FILLING_RETURN) == ORDER_FILLING_RETURN) {
        Print("RETURN filling is supported");
        return ORDER_FILLING_RETURN;
    }
    if ((filling_mode & ORDER_FILLING_BOC) == ORDER_FILLING_BOC) {
        Print("BOC filling is supported");
        return ORDER_FILLING_BOC;
    }
    
    // Default fallback
    Print("No specific filling mode found, using FOK as default");
    return ORDER_FILLING_FOK;
}